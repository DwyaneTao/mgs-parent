...
public class JwtUtil {
    ...
    //服务端私钥
    public static final String JWT_SECRET = "ZFN#k%Mx16jC5x#f";

    //jwt过期时间
    public static final int JWT_TTL = 7*24*60*60*1000;  //millisecond//8h
    ...
    public static String createJWT(Jwt jwt) throws Exception {
        ...
        JwtBuilder builder = Jwts.builder()
                ...
                .claim("userAccount",jwt.getUserAccount())
                .claim("userName",jwt.getUserName())
                .claim("orgCode",jwt.getOrgCode())
                .claim("domainName",jwt.getDomainName())
                .claim("hotelInfoPermission",jwt.getHotelInfoPermission())
                //jwt的签发时间
                .setIssuedAt(now)
                //代表这个JWT的主体，即它的所有人，这个是一个json格式的字符串，可以存放什么userid之类的，作为什么用户的唯一标志。
                .setSubject(jwt.getUserAccount())
                //设置签名使用的签名算法和签名使用的秘钥
    ...
    public static Jwt parseJWT(String token) {
        ...
        try{
            ...
            jwt.setUserAccount((String) claims.get("userAccount"));
            jwt.setUserName((String) claims.get("userName"));
            jwt.setOrgCode((String) claims.get("orgCode"));
            jwt.setDomainName((String) claims.get("domainName"));
            jwt.setHotelInfoPermission((Integer) claims.get("hotelInfoPermission"));
            jwt.setResult(1);
        }catch (SignatureException e){
            log.error("token解析失败",e);
            jwt.setResult(0);
            jwt.setParseResCode(ErrorCodeEnum.FAIL_SIGNATURE.errorCode);
